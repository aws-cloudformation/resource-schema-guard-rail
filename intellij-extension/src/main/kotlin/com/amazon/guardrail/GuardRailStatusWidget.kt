        ...
        override fun getTooltipText(): String {
            return when {
                errorCount > 0 && warningCount > 0 -> 
                    "Guard Rail validation found $errorCount error(s) and $warningCount warning(s)"
                errorCount > 0 -> 
                    "Guard Rail validation found $errorCount error(s)"
                warningCount > 0 -> 
                    "Guard Rail validation found $warningCount warning(s)"
                else -> 
                    "Guard Rail validation passed with no issues"
            }
        }
