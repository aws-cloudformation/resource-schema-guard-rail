...
class GuardRailDiagnosticMapper {
    ...
    private fun navigatePath(element: PsiElement?, segments: List<String>, root: PsiElement?): PsiElement? {
        ...
        val next = when {
            ...
            segment == "*" && current is PsiJsonObject -> 
                current.findProperty("items")?.value
            
            // Numeric index - navigate into array
            segment.toIntOrNull() != null && current is JsonArray -> 
                current.valueList.getOrNull(segment.toInt())
